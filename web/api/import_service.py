...
async def upload_files(background_tasks: BackgroundTasks, files: List[UploadFile] = File(...)):
    ...
    for file in files:
        ...
        print(f"[{task_id}] 开始处理上传文件，文件名：{file.filename}，文件类型：{file.content_type}")
        ...
        print(f"[{task_id}] 文件已保存至本地，路径：{import_file_path}")
        ...
        print(f"[{task_id}] 已将LangGraph全流程加入后台任务，任务已启动")

    # 9. 所有文件处理完毕，返回上传成功信息和所有TaskID（前端基于TaskID轮询进度）
    print(f"多文件上传处理完毕，共处理{len(files)}个文件，生成TaskID列表：{task_ids}")
    return {
        "code": 200,
